			...
			foreach($settings['slider'] as $slide_id => $slide){
				if( ($current_size >= ($paged - 1) * $settings['num-fetch']) &&
					...
					if( !empty($current_size) && ($current_size % $settings['gallery-columns'] == 0) &&
						(empty($settings['with-featured']) || $settings['with-featured'] == 'disable') ){

						$ret .= '<div class="clear"></div>';
					}else if( (!empty($settings['with-featured']) && $settings['with-featured'] == 'enable') ){
						$count_size = $current_size;
						$row = intval($current_size / $settings['gallery-columns']);
						if( $row != 0 ){ 
							$count_size += 3;
						}

						if( $row !== 0 && $count_size % $settings['gallery-columns'] == 0 ){
							$ret .= '<div class="clear"></div>';
						}
					}
					if( !empty($settings['with-featured']) && $settings['with-featured'] == 'enable' && $current_size == 0 ){
						$ret .= '<div class="gallery-column ' . gdlr_get_column_class('2/' . $settings['gallery-columns']) . '">';	
					}else{
